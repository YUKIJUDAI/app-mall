...
router.post('/register', async (ctx) => {
  let { userName, password, mobilePhone } = ctx.request.body;
  
  if (!userName || !password || !mobilePhone) return ctx.body = { code: 4020, msg: '请输入完整信息' };
...
router.post('/login', async (ctx) => {
  let { mobilePhone , password } = ctx.request.body;
  
  if (!mobilePhone || !password ) return ctx.body = { code: 4020, msg: '请输入完整信息' };
