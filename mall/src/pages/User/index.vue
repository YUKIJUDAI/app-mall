<template>
    <div class="user">
        ...
        <section class="other-entry">
            <!-- 商品管理 -->
            <div class="collection" @click="$router.push({ name: 'Management' })" v-if="userInfo.type === 2">
                <van-icon class="small-icon" name="description" />
                <span>商品管理</span>
    ...
    methods: {
        ...
        async _getUserInfo() {
            try {
                ...
                if (res.code === 0) this.userInfo = res.data[0];
            } catch (error) {
