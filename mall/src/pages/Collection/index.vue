...
export default {
    ...
    methods: {
        ...
        async _getCollectionList(isLoadMore) {
            ...
            // 上锁，方法在 loadMixin 中
            this.locked();
            const res = await ajax.getCollectionList();
            this.loadingStatus = false;
            this.unLocked();
            if (res.code === 0) {
                // this.setDataTotal(res.total); // 设置数据总数，方法在 loadMixin 中
                // 判断是加载更多还是一次新的请求，方法在 loadMixin 中
                // isLoadMore ? this.addMoreData(res.data) : this.dataList = res.data;
        ...
        async cancelOneCollection(item, index) {
            let res = await ajax.collectionCancleHandle(item.goods_id); // 0: 指取消收藏
