<template>
    <div>
        ...
        <!-- 内容区 -->
        <section class="content" v-if="hotList" @touchstart.prevent="touchstart" @touchmove.prevent="touchmove" @touchend.prevent="touchend">
            <b-scroll class="content-scroll" ref="homeScrollRef" v-if="hotList" :data="hotList" :probeType="probeType" :pullup="true" :bounce="bounce" :listenScroll="true" v-on:scroll="contentScroll" v-on:scrollEnd="homeScrollEnd">
                <div class="container">
                    <!-- 轮播图 -->
                    <banner :slides="bannerList"></banner>
                    <!-- 分类 -->
                    ...
                    <!-- <div v-if="homeData.floorName">
                        <floor v-for="(item, index) in Object.keys(homeData.floorName)" :key="index" :floorName="homeData.floorName[item]" :floor="homeData[item]" :num="index + 1">
                        </floor>
                    </div> -->
                    <!-- 热卖商品 -->
                    <hot-goods :hotGoods="hotList"></hot-goods>
                </div>
            </b-scroll>
        ...
        <!-- 搜索结果 -->
        <search v-show="isShowSearch" :searchResult="dataList" :searchKeyword="searchKeyword" :searchStatus="searchStatus" :isEmptySearchResult="isEmptySearchResult" :isloadMore="isloadMore" v-on:click-search="clickSearch" v-on:scrollEnd="searchScrollEnd">
        </search>
    </div>
</template>
...
export default {
    ...
    methods: {
         ...
         */
        async _getHomeAds() {
            let res = await ajax.getHomeAds({ type: 1 });
            if (res.code === 0) {
        ...
        async _search(keyword, isLoadMore) {
            ...
            try {
                let res = await ajax.search({ keyword });
                if (res.code === 0) {
                    // 分页配置
                    // this.setDataTotal(res.result.total); // 设置数据总数，方法在 loadMixin 中
                    // 判断是加载更多还是一次新的请求，方法在 loadMixin 中
                    // isLoadMore ? this.addMoreData(res.result.goodsList) : this.dataList = res.result.goodsList;
                    this.dataList = res.data.goods_search_response.goods_list;
