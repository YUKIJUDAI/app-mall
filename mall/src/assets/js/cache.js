...
const SHOPPING_CART_LIST_INFO = 'shopping_cart_list'; // 购物车列表
 ...
 * 购物车 缓存
 */
let shoppingCartListCache = {
    setCache(shoppingCartListInfo) {
        var oldData = storage.getItem(SHOPPING_CART_LIST_INFO) ? JSON.parse(storage.getItem(SHOPPING_CART_LIST_INFO)) : [];
        oldData.filter(item => item.goods_id === shoppingCartListInfo.goods_id).length === 0
            ? oldData.push(shoppingCartListInfo)
            : oldData.forEach((item, i) => { item.goods_id === shoppingCartListInfo.goods_id && item.num++ });
        storage.setItem(SHOPPING_CART_LIST_INFO, JSON.stringify(oldData));
        return oldData;
    },
    updateCache(shoppingCartListInfoList) {
        storage.setItem(SHOPPING_CART_LIST_INFO, JSON.stringify(shoppingCartListInfoList));
        return shoppingCartListInfoList;
    },
    getCache() {
        return storage.getItem(SHOPPING_CART_LIST_INFO) ? JSON.parse(storage.getItem(SHOPPING_CART_LIST_INFO)) : [];
    },
    deleteCache() {
        storage.removeItem(ADDRESS_INFO);
        return {};
